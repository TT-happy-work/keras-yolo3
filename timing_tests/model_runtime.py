...
def _main():
    ...
    val_inputs = []
    for i in range(10):
        tmp_input = val_inputs_generator.__next__()[0]
        if data_format == 'channels_last':
            val_inputs.append(tmp_input[0])
        elif data_format == 'channels_first':
            val_inputs.append(np.moveaxis(tmp_input[0], -1, 1))
    ...
    with tf.Session(graph=graph) as sess:
        for i in range(10):
            sess.run(outputs, feed_dict={input_tensor: val_inputs[i]})

        runtimes = []
        for i in range(10):
            start_time = time.perf_counter()
            sess.run(outputs, feed_dict={input_tensor: val_inputs[0]})
